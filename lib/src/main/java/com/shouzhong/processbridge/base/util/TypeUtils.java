...
import com.shouzhong.processbridge.base.annotation.WithinProcess;
...
public class TypeUtils {
    ...
    public static Method getMethod(Class<?> clazz, String methodName, Class<?>[] parameterTypes, Class<?> returnType)
        ...
        for (Method method : methods) {
            if (method.getName().equals(methodName) && classAssignable(method.getParameterTypes(), parameterTypes)) {
                if (result == null) {
                    ...
                    throw new ProcessBridgeException(ErrorCodes.TOO_MANY_MATCHING_METHODS,
        ...
        if (result.getReturnType() != returnType) {
            throw new ProcessBridgeException(ErrorCodes.METHOD_RETURN_TYPE_NOT_MATCHING,
    ...
    public static Method getMethodForGettingInstance(Class<?> clazz, String methodName, Class<?>[] parameterTypes)
            ...
            if (methodName.equals("") && (tmpName.equals("getInstance") || method.isAnnotationPresent(GetInstance.class))
                ...
                if (classAssignable(method.getParameterTypes(), parameterTypes)) {
                    if (result == null) {
                        ...
                        throw new ProcessBridgeException(ErrorCodes.TOO_MANY_MATCHING_METHODS_FOR_GETTING_INSTANCE,
        ...
        if (result != null) {
            if (result.getReturnType() != clazz) {
                throw new ProcessBridgeException(ErrorCodes.GETTING_INSTANCE_RETURN_TYPE_ERROR,
        ...
        throw new ProcessBridgeException(ErrorCodes.GETTING_INSTANCE_METHOD_NOT_FOUND,
    ...
    public static Constructor<?> getConstructor(Class<?> clazz, Class<?>[] parameterTypes) throws ProcessBridgeException {
        Constructor<?> result = null;
        Constructor<?>[] constructors = clazz.getConstructors();
        for (Constructor<?> constructor : constructors) {
            if (classAssignable(constructor.getParameterTypes(), parameterTypes)) {
                if (result != null) {
                    throw new ProcessBridgeException(ErrorCodes.TOO_MANY_MATCHING_CONSTRUCTORS_FOR_CREATING_INSTANCE,
        ...
        if (result == null) {
            throw new ProcessBridgeException(ErrorCodes.CONSTRUCTOR_NOT_FOUND,
    ...
    public static ParameterWrapper[] objectToWrapper(Object[] objects) throws ProcessBridgeException {
        if (objects == null) {
        ...
        for (int i = 0; i < length; ++i) {
            try {
                ...
                e.printStackTrace();
                throw new ProcessBridgeException(e.getErrorCode(),
    ...
    public static void validateAccessible(Class<?> clazz) throws ProcessBridgeException {
        if (clazz.isAnnotationPresent(WithinProcess.class)) {
            throw new ProcessBridgeException(ErrorCodes.CLASS_WITH_PROCESS,
                    "Class " + clazz.getName() + " has a WithProcess annotation on it, "
                            + "so it cannot be accessed from outside the process.");
        }
    }

    public static void validateAccessible(Method method) throws ProcessBridgeException {
        if (method.isAnnotationPresent(WithinProcess.class)) {
            throw new ProcessBridgeException(ErrorCodes.METHOD_WITH_PROCESS,
    ...
    public static void validateAccessible(Constructor<?> constructor) throws ProcessBridgeException {
    ...
    public static void methodParameterTypeMatch(Method method, MethodWrapper methodWrapper) throws ProcessBridgeException {
        ...
        if (requiredParameterTypes.length != parameterTypes.length) {
            throw new ProcessBridgeException(ErrorCodes.METHOD_PARAMETER_NOT_MATCHING,
    ...
    public static void methodReturnTypeMatch(Method method, MethodWrapper methodWrapper) throws ProcessBridgeException {
    ...
    public static void methodMatch(Method method, MethodWrapper methodWrapper) throws ProcessBridgeException {
